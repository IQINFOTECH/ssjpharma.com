<?php
/** @var array $area @var array $products @var int $total @var int $page @var int $totalPages @var array $breadcrumbs */
...
<section class="container-x pb-12">
  <?php if (empty($products)): ?>
    <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-16 text-center text-slate-500">No products in this therapeutic area yet.</div>
  <?php else: ?>
    ...
    <nav class="mt-10 flex flex-wrap justify-center gap-2" aria-label="Pagination">
      <?php for ($i = 1; $i <= $totalPages; $i++): ?><a href="/therapeutic-area/<?= e($area['slug']) ?>?page=<?= $i ?>" class="rounded-lg border px-4 py-2 text-sm <?= $i === $page ? 'border-brand-500 bg-brand-500 text-white' : 'border-slate-200 text-slate-600 hover:bg-slate-50' ?>"><?= $i ?></a><?php endfor; ?>
    </nav>
    <?php endif; ?>
  <?php endif; ?>
</section>
